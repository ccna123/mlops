...
| ------------------------------------ | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| API layer cho Dashboard              | FastAPI (Python) —`services/api/`            | Đứng giữa Dashboard và Airflow/MLflow/MinIO, trả JSON; auth hoãn, xem dòng Auth bên dưới                               |
...
| `house_needs_renovation_classifier` | Classification (binary) | `needs_renovation` (sinh từ `condition` ∈ poor/fair) | **AUC** (cổng), kèm F1 và accuracy để báo cáo | `condition`, `days_on_market`, `sale_price`, `sold_within_30_days`, `price_category` |

Hai bài toán này đo hai thứ khác nhau (giá bán vs tình trạng tài sản) nên bổ sung cho nhau. `price_category` không được chọn làm target vì nó suy trực tiếp ra từ `sale_price`, tức là lặp lại bài regression.

**Bài toán classification đã đổi ở Plan 3, sau khi đo.** Bản v2 chọn `sold_within_30_days`. Đo trên 48.000 dòng thật cho thấy nó phụ thuộc gần như hoàn toàn vào `days_on_market` — mà cột đó bắt buộc phải loại vì target suy trực tiếp ra từ nó. Bỏ đi thì toàn bộ 19 feature còn lại gộp lại chỉ đạt AUC 0.58. Target mới `needs_renovation` đạt **AUC 0.71**, và quan trọng hơn là tín hiệu **phân tán thật**: cột đơn lẻ mạnh nhất chỉ 0.56, phần còn lại gần như đúng 0.50. Xem mục 2.5 của `docs/superpowers/specs/2026-09-19-plan3-serving-design.md`.

**Cột `list_price` cần một quyết định riêng, không phải leakage.** Tại thời điểm dự đoán, giá rao bán đã biết — dùng nó là hợp lệ. Nhưng `sale_price ≈ list_price` với sai số nhỏ, nên đưa vào là bài toán trở nên tầm thường: model sẽ chỉ học một hệ số nhân và R² gần 1 mà không học được gì từ các feature còn lại. Giai đoạn 1 **loại `list_price` khỏi feature** của model regression để bài toán có nội dung thật; giữ lại cho model classification vì tại thời điểm đăng tin giá rao đã biết, và nó là feature đơn lẻ mạnh nhất của bài toán đó (AUC 0.56).
...
| Models             | Xem model version (tên, version, metric, version nào đang giữ alias `champion`), chuyển alias `champion` sang version khác                                           | **Cột metric động theo loại model**: rmse/mae/r2 cho regression, f1/auc/accuracy cho classification — không hardcode accuracy/f1 nữa |
...
| `GET`  | `/api/models`                          | Danh sách registered model + version + metric + version đang giữ alias `champion` (`is_champion`)| MLflow REST                               |
...
- `dataset_version`: chuỗi thường, mặc định `v1`. **API không kiểm nó** (khác `POST /api/data/upload`, nơi tên version phải khớp một pattern): nó được chuyển nguyên vào `conf` của lần chạy. Một version chưa có `raw/<version>/data.parquet` vẫn được xếp hàng (`queued`) và chỉ hỏng khi task `extract` chạy (`FileNotFoundError`; suy ra từ code `stages/extract/main.py`, chưa chạy thật).
